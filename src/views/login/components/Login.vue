...
const rules = reactive({
	account: [{ required: true, message: '请输入您的账号', trigger: ['blur', 'change'] }],
	password: [{ required: true, message: '请输入您的密码', trigger: ['blur', 'change'] }]
})
...
<template>
	<div class="login-form-wrapper">
		...
		>
			...
			<el-button class="button" size="large" @click="submitForm(ruleFormRef)" type="primary"
				>立即登录</el-button
			>
		</el-form>
		<div class="other flex-bt">
			<div class="left">
				没有账号？
				<span class="pointer" @click="$emit('changeActive', 'Register')">立即注册</span>
			</div>
			<div class="right pointer" @click="$emit('changeActive', 'ResetPassword')"
				>忘记密码</div
			>
		</div>
	</div>
