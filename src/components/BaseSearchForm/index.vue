...
const measureTextWidth = (text) => {
    ...
    ctx.font = '14px  Helvetica Neue, Helvetica, PingFang SC, Hiragino Sans GB, Microsoft YaHei, Arial, sans-serif'; // 设置字体样式
...
const isTimerPicker = (type) => {
    const TIME_TYPE_ENUM = ['year', 'month', 'date', 'dates', 'datetime', 'week', 'datetimerange', 'daterange', 'monthrange']
...
<template>
    <el-form :model="data">
        <div class="base-search-form-wrapper flex-start">
            <div class="items flex-start" v-for="item in config" :key="item.key">
                <span class="label" :style="{ width: maxLabelWidth + 'px' }">
                    {{ item.label }}
                </span>
                <div class="item-content">
                    <!-- input -->
                    <el-input v-if="item.type === 'input'" v-model="data[item.key]" :placeholder="item.placeholder"
                        :clearable="clearable" />
                    <!-- select -->
                    <el-select v-if="item.type === 'select'" v-model="data[item.key]" :placeholder="item.placeholder"
                        :filterable="item.filterable || true" :clearable="clearable">
                        <el-option v-for="child in item.options" :key="child.value" :label="child.label"
                            :value="child.value" />
                    </el-select>
                    <!-- datePicker -->
                    <el-date-picker v-if="isTimerPicker(item.type)" :editable="item.editable || false"
                        v-model="data[item.key]" :placeholder="item.placeholder" :start-placeholder="item.startPlaceholder"
                        :end-placeholder="item.endPlaceholder" :type="item.type" :format="getFormat(item.type, item.format)"
                        :value-format="item.valueFormat || 'x'" :disabled-date="item.disabledDate" />
                    <!--interval  -->
                    <div v-if="item.type === 'interval'" class="interval-box flex-bt">
                        <el-input v-model="data[item.key[0]]" :placeholder="item.startPlaceholder" :clearable="clearable" />
                        <span class="centerText">{{ item.centerText || '至' }}</span>
                        <el-input v-model="data[item.key[1]]" :placeholder="item.endPlaceholder" :clearable="clearable" />
                    </div>
                </div>
            </div>
            <div class="items">
